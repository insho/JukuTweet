...
public class MenuExpandableListAdapter extends BaseExpandableListAdapter {
    ...
    public MenuExpandableListAdapter(Context context, List<String> listDataHeader,
                                     ...
                                     Boolean usecolorbar, List<Integer> addStarList,
    ...
    public View getChildView(int groupPosition, final int childPosition,
        ...
        if(debug){Log.d(TAG, "listdataextra size: " + _listDataExtra.size());}
        ...
        if(_listDataExtra != null &&  _listDataExtra.size() > 0 && _listDataExtra.containsKey(newHeaderTitle) && childText.equalsIgnoreCase("Browse/Edit")) {
            ...
            if (red + yellow + green == 0) {
                ...
                if(debug){Log.d(TAG, newHeaderTitle+ " - " + "grey (total) dimenscore: " + dimenscore_total);}
    ...
    public View getGroupView(int groupPosition, boolean isExpanded,
        ...
        if(debug){Log.d(TAG,"headerTitle: --" + headerTitle + "--" );}
        if(_textsize>0){
        ...
        if(_usecolorbar) {
            if(debug){Log.d(TAG,"USING COLORBAR"  );}

            Integer colorcount = _listDataExtra.get(headerTitle).get(1);
            lblColorBar = (TextView) convertView.findViewById(R.id.lblcolorbar);
            lblColorBar.setText(String.valueOf(colorcount));
            ...
            if (_mylists) {  //.i.e. we're expanding the favorites lists in myfavorites, and there are star pictures to assign to the headers
                ...
                if (_addStarList.size() > groupPosition && _colors.contains(headerTitle)) {
                    ...
                    lblListHeader.setAlpha(1.0f);
                    if(headerTitle.equalsIgnoreCase("Create New List") || groupPosition == _listDataHeader.size() -1) {
                        if(debug){Log.d(TAG, "doing the correct thing");}

                        lblListHeaderCount.setVisibility(TextView.GONE);
                    } else if(count>0) {
                        if(debug){Log.d(TAG,"setting counts...");}
                    ...
                    if(!_listDataExtra.containsKey(newHeaderTitle) && headerTitle.equalsIgnoreCase("Create New List")) {
                        ...
                        if(debug){Log.d(TAG, "doing the correct thing 2");}
                        ...
                        lblListHeader.setAlpha(1.0f);
                        lblListHeaderCount.setAlpha(.7f);
                        if(debug){Log.d(TAG, "?? 4  -- " + headerTitle.toString());}
            ...
            if(_mylists && !_usecolorbar) {//i.e. if it's the favorites list don't do any of that shit
                ...
                if (_listDataExtra != null && _listDataExtra.size() > 0 && !headerTitle.equalsIgnoreCase("Create New List")) {
                    ...
                    if(debug){Log.d(TAG,headerTitle+ " - " + "groupview dimenscore total: " + dimenscore_total);}
                    if (_mylists) {
                        dimenscore_total = Math.round((dimenscore_total * ((float) 0.866666667 / (float) dimenscore_total)));
                        if(debug){Log.d(TAG,headerTitle+ " - " + "groupview dimenscore decreasing to : " + dimenscore_total);}
                    }
                    ...
                    if (red + yellow + green == 0) {
                        ...
                        textViewColorBlock_grey.setVisibility(View.VISIBLE);
                        textViewColorBlock_grey.setMinimumWidth(dimenscore_total);
                        if(debug){Log.d(TAG, headerTitle+ " - " + "grey (total) dimenscore: " + dimenscore_total);}
        ...
        if(debug){Log.d(TAG,"dimenscore at end of group: " + dimenscore_total);}
        return convertView;
    }
